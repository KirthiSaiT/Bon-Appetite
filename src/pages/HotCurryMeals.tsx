...
const HotCurryMeals = () => {
  ...
  const [selectedCurry, setSelectedCurry] = useState<{name: string, description: string, price: string, image: string} | null>(null);
  ...
  const handleOrderClick = (curry: {name: string, description: string, price: string, image: string}) => {
  ...
  const handleSubmitOrder = () => {
    ...
    const templateParams = {
      ...
      message: `
        You have a new hot curry meal order!
        Product: ${selectedCurry?.name}
        Price: ${selectedCurry?.price}
        Quantity: ${orderForm.quantity}
        Description: ${selectedCurry?.description}
  ...
  const hotCurryMeals = [
    {
      ...
      price: "₹400",
      image: "/lovable-uploads/hot curry.jpg",
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-orange-50 to-red-50">
            ...
            {hotCurryMeals.map((curry, index) => (
              <div key={index} className="bg-white rounded-xl shadow-lg hover:shadow-xl transition-shadow duration-300 overflow-hidden">
                ...
                <div className="p-6">
                  ...
                  <p className="text-gray-600 text-sm mb-4 leading-relaxed">{curry.description}</p>
                  ...
                  <div className="flex items-center justify-between mb-4">
                    <span className="text-lg font-bold text-orange-600">{curry.price}</span>
                  </div>
                  
                  <button
                    onClick={() => handleOrderClick(curry)}
      ...
      {showOrderModal && selectedCurry && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl max-w-md w-full max-h-[90vh] overflow-y-auto">
            <div className="sticky top-0 bg-white border-b border-gray-200 p-6 rounded-t-xl">
              <div className="flex items-center justify-between">
                <h3 className="text-xl font-bold text-gray-800">Order a Hot Curry Meal</h3>
                <button 
                  onClick={closeModal}
            ...
            <div className="p-6">
              <div className="bg-orange-50 rounded-lg p-4 mb-6">
                <h4 className="font-semibold text-orange-800 mb-1">{selectedCurry.name}</h4>
                <p className="text-sm text-orange-700">{selectedCurry.price}</p>
                <p className="text-xs text-orange-600 mt-1">{selectedCurry.description}</p>
              </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl p-8 max-w-md w-full text-center">
            ...
            <p className="text-gray-600 mb-4">
              Thank you for your curry order! We'll contact you soon.
            </p>
          </div>
        </div>
      )}
